...
sed -i \
    -e "s/\${TRAEFIK_LOG_LEVEL}/${TRAEFIK_LOG_LEVEL:?empty or missing envvar}/g" \
    -e "s/\${TRAEFIK_DOCKER_NETWORK}/${TRAEFIK_DOCKER_NETWORK:?empty or missing envvar}/g" \
...
echo "Watching /certs/localtest.pem for changes to reload Traefik config ..."
touch /etc/traefik/certs.log
inotifywait /certs/localtest.pem -d -q -o /etc/traefik/certs.log -e modify --format '[%T] %|e %w%f' --timefmt '%F %T'
